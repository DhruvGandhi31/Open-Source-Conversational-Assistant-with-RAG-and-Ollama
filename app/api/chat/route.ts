...
/**
 * Get embeddings for a query using Ollama embeddings API.
 */
const getEmbedding = async (question: string): Promise<number[]> => {
    const embeddingModel = new OllamaEmbeddings({
        model: 'nomic-embed-text:latest',
        baseUrl: process.env.OLLAMA_HOST, // e.g. http://127.0.0.1:11434
    })

    try {
        return await embeddingModel.embedQuery(question)
    } catch (error) {
...
/**
 * Lookup knowledge from Postgres vector store.
 */
